...
class InMemoryDatabaseTest extends Specification {
    ...
    Company from = new Company(1L, 1L, "address1" );
    Company to = new Company(2L, 2L, "address2");
    ...
    def "should save invoice to database"() {
        ...
        inMemoryDatabase.database.get(invoice.getId()) == invoice;
    }
    ...
    def "should return invoice by id"() {
        ...
        inMemoryDatabase.database.put(invoice.getId(), invoice);

        when: "we ask for an invoice by id"
        def returnedInvoiceOptional = inMemoryDatabase.getById(invoice.getId());

        then: "invoice is returned"
        returnedInvoiceOptional.get() == invoice;
    }

    def "should return empty optional when there is no invoice with this id"() {
        when: "we ask database for an nonexistent invoice"
        def returnedInvoiceOptional = inMemoryDatabase.getById(UUID.randomUUID());

        then: "null is returned"
        returnedInvoiceOptional.isEmpty()
    }
    ...
    def "should return list of all invoices from database"() {
        ...
        def ArrayList<Invoice> invoiceList = inMemoryDatabase.getAll();
    ...
    def "should update invoice in database"() {
        ...
        Company updatedFromCompany = new Company(3L, 3L, "address3")
        Company updatedToCompany = new Company(4L, 4L, "address4")
